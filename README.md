...
## 4. 管理員後台重新美化（與前台同一套視覺）
- 沿用前台的深色卡片式設計：同樣的底色、圓角卡片、藍色主按鈕、紅色刪除鈕。
- 登入後分成「檔案」與「使用者」兩個分頁，卡片標題會顯示筆數。
- 帳號身分用膠囊標籤區分（管理員為藍色）。
- **未登入時顯示「鎖住」的檔案清單**：檔名以方塊字元遮蔽、右側標示「未授權」，
  登入表單放在下方。這些是**純佔位資料**，寫死在前端，不會向伺服器索取，
  也不會洩漏任何真實檔名；真正的清單要登入成功後才會透過 `/api/admin/files` 取得。

## 5. 一般使用者也會閒置自動登出
- 前台加上與後台相同的 300 秒（5 分鐘）閒置自動登出。
- 標題列下方會即時顯示倒數秒數，剩下 60 秒時轉成黃色提醒。
- 有任何滑鼠、鍵盤、觸控或捲動操作就會重新計時；自動登出後會回到登入畫面並顯示原因。
